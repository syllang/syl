...
{
    pub(super) fn static_type_name<'b>(&self, ty: &'b MirTypeRef) -> Option<&'b str> {
        ty.type_name()
    }

    pub(super) fn first_type_arg<'b>(&self, ty: &'b MirTypeRef) -> Option<&'b MirTypeRef> {
        ty.args()?.first()
    }
    ...
    pub(super) fn enum_width(&self, owner: Option<DefId>, item: &ElabEnumItem) -> String {
        item.width
            .as_ref()
    ...
    pub(super) fn enum_width_expr(&self, owner: Option<DefId>, item: &ElabEnumItem) -> EirExpr {
        item.width
            .as_ref()
            .map(|width| {
