use super::summary::CellSummaryCollector;
use crate::{
...
impl<'a> DriverFactsCollector<'a> {
    ...
    fn collect_creates(&mut self) {
        for object in self.eir.objects() {
            ...
            self.creates.push(CreateFact::new(
                object.module(),
                object.name(),
                object_id,
                kind,
    ...
    fn collect_source_instance_summaries(&self) -> Vec<DriverCellSummary> {
    ...
    fn collect_source_instance_summaries_from_items(
        &self,
        items: &[EirItem],
        summaries: &mut BTreeMap<DriverSummaryOriginKey, DriverCellSummary>,
    ) {
        for item in items {
            match item {
