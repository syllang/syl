use crate::{
    ...
    hir::{
        HirBodyExpr, HirCallArg, HirDefKind, HirDesign, HirExprNode, HirMapItem, HirMatchArm,
        HirNamedExpr, HirSelectArm, view::HirDesignViewExt,
    },
    ir::mir::MirPattern,
    tir::{BuiltinIntrinsic, BuiltinResolver, TirDesign, TirGenericArg, TirType},
};
...
pub(crate) trait MapLoweringContext {
    ...
    fn builtin_intrinsic(&self, owner: DefId, callee: &HirBodyExpr) -> Option<BuiltinIntrinsic>;
...
impl MapLoweringContext for TirDesign {
    fn hir(&self) -> &HirDesign {
        TirDesign::hir(self)
    }
    ...
    fn expr_type(&self, owner: DefId, expr: &HirBodyExpr) -> Option<&TirType> {
        let id = match &expr.node {
            HirExprNode::Ident(_) => TirDesign::hir(self)
    ...
    fn builtin_intrinsic(&self, owner: DefId, callee: &HirBodyExpr) -> Option<BuiltinIntrinsic> {
        BuiltinResolver::new(TirDesign::hir(self), Some(owner)).resolve_call_callee(callee)
    }
    ...
    fn extension_method_call<'a>(
        ...
        TirDesign::extension_method_call(self, owner, callee)
            .map(|call| (call.method, call.receiver, call.inferred_args))
    }
    ...
    fn def_kind(&self, def: DefId) -> Option<HirDefKind> {
        TirDesign::hir(self).def_kind(def)
    }

    fn def_name(&self, def: DefId) -> Option<&str> {
        TirDesign::hir(self).def_name(def)
    }
}
...
impl<'a> MapIrBuilder<'a> {
    ...
    #[cfg(test)]
    pub(crate) fn with_context(ctx: &'a dyn MapLoweringContext) -> Self {
        Self { ctx }
    }
    ...
    fn lower_expr(&self, owner: DefId, expr: &HirBodyExpr) -> Result<MapExpr, CompileError> {
        let lowered = match &expr.node {
            ...
            HirExprNode::Field { base, field } => {
                if let Some(value) = self.ctx.enum_variant_value(expr) {
                    MapExpr::Int(value)
                } else {
                    MapExpr::Field {
    ...
    fn lower_call(
        ...
        if let Some((method, receiver, inferred_args)) =
            self.ctx.extension_method_call(owner, callee)
        {
            let mut lowered_args = vec![MapArg::new(None, self.lower_expr(owner, receiver)?)];
            lowered_args.extend(
                args.iter()
                    .map(|arg| {
                        Ok(MapArg::new(
            ...
            if generic_args.is_empty() {
                generic_args = inferred_args.iter().map(MapGenericArg::from).collect();
            }
        ...
        match self.ctx.builtin_intrinsic(owner, callee) {
        ...
        let Ok(Some(HirResolution::Def(def))) = self.ctx.expr_resolution(owner, root) else {
            return Err(CompileError::lowering_at(
                EirError::UnknownHardwareValueCall { name: callee_name },
                callee.span(),
            ));
        };
        if self.ctx.def_kind(def) == Some(crate::hir::HirDefKind::Map) {
            Ok(MapExpr::Call {
                callee: def,
                generic_args,
                args: lowered_args,
    ...
    fn lower_match_arms(
        ...
        let target_enum_name = self.match_target_enum_name(owner, value);
        arms.iter()
            .map(|arm| {
                Ok(MapMatchArm::new(
    ...
    fn lower_match_pattern(
        ...
        match pattern {
            MirPattern::Path(path, _) if path.len() == 1 => {
    ...
    fn match_target_enum_name(&self, owner: DefId, value: &HirBodyExpr) -> Option<String> {
        let ty = self.ctx.expr_type(owner, value)?;
        let def = ty.definition()?;
        (self.ctx.def_kind(def) == Some(crate::hir::HirDefKind::Enum))
            .then(|| self.ctx.def_name(def).map(str::to_string))
            .flatten()
    }
