...
fn architecture_phase1_ir_owners_stay_single_source() {
    ...
    let hw_parametric = read_text(&workspace.join("crates/syl_hw/src/parametric.rs"));
    for forbidden in ["Eir", "DriverFacts", "TypePhaseChecker", "ConstMir"] {
        assert!(
            !hw_parametric.contains(forbidden),
            "HW IR must not carry semantic/elaboration temporary state: {forbidden}"
        );
    }
}
