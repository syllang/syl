...
执行时，必须遵照 AGENTS.md。S1 主 Agent 先派出任务给 Work SubAgent（使用 gpt-4-xhigh），然后 S2 Review SubAgent （ gpt-4-medium）检查并反馈问题，S3 主 Agent 审核和仔细分析后传达给 Work SubAgent 整改。S4 整改完之后 Review SubAgent 重复检查(S2~S4)……这是循环过程，直到本 Phase 问题收敛。每个 S 步骤，在对应 .tmp/rfc/roadmap.md 的对应 Phase下标记一个 Log 行。整个Phase 问题收敛完成后只有 Review Agent 有权独立地标记为完成，然后才能进入下一个 Phase.
  ...
  - 2026-05-24 S2 - Review SubAgent 判定 Phase 9 未收敛：`cargo test --workspace --all-targets --no-fail-fast` 中 `syl_lsp::tests::diagnostic_publications_use_token_aware_grouped_diagnostics` 仍出现 flaky 失败；`git diff edad16e^..edad16e --check` 发现 `conformance/backend/positive/passthrough.sv` EOF whitespace，且 `quality_gate.sh` 只检查工作区 diff，不能发现已提交范围内的 whitespace；public API checker 仍是源码 regex 扫描，不能解析真实 crate public surface、re-export、cfg/features，consumer policy 也不是逐 API 级；`check_examples.sh` 在缺少 Verilator 时返回 0，导致必须质量线可被 skip。
  - 2026-05-24 S3 - 主 Agent 审核 Review 结论后派 Work SubAgent 整改：彻底稳定 LSP cancellation 测试而非延长等待；修复 conformance SV whitespace 并让 quality gate 检查提交范围或 baseline 范围的 committed whitespace；将 public API surface checker 改为基于 `cargo doc`/rustdoc JSON/`cargo public-api` 等真实 crate surface 的结构化快照，并把 consumer policy 细化到新增 public item；Verilator 缺失必须 fail fast，不能作为 Phase 9 必须质量线被 skip。
  - 2026-05-24 S4 - Work SubAgent 完成 S3 整改：LSP cancellation 用例去除 helper 线程竞态，query 层用包完成 observer 单线程证明 alpha 后取消不会启动 beta package diagnostics；修复 `passthrough.sv` EOF whitespace，`quality_gate.sh` 增加 baseline/提交范围 whitespace 检查且缺少 origin 时明确 fallback；`check_public_api.py` 改为 rustdoc JSON 解析真实 exported surface 并要求 `api/public-api-consumers.md` 逐 surface line 说明消费者；`check_examples.sh` 在缺少 Verilator 时 fail fast 并输出安装指引。
